...
from globe.graphUtil import GraphUtil;
...
class DAG:
	...
	def ForwardSearch(self,resume=False):
		dict_=self.dict;
		skips=0
		...
		while not self.priority_queue.empty() :
			...
			if resume:
				print('Considering Edge from : '+str(source_node)+ ' to: '+ str(target_node)+ ' costing: '+ str(edge_cost));
			#------------------------------SANITY CHECKS-----------------------------------#
			...
			if flags[0]:
				...
				if resume: print("Edge already part of the graph");
			if flags[1]:
				self.logger.WriteLog("stale node");
				if resume: print("stale node");
			if flags[2]: 
				self.logger.WriteLog("Edge is cyclic");
				if resume: print("Edge is cyclic");
			if any(flags):	
			...
			self.Nodes[target_node].SetCurrentModel(candidate_edge.GetModel())
		...
		for target_node in range(len(self.Nodes)):
			...
			while edge_removed:
				...
				for set_ in sets:
					...
					gained_ratio,updated_cost,model,absolute_gain= self.globe_.GetCombinationCost(parent_nodes,current_edges,child);
				...
				if edge_removed:
					...
					self.Nodes[target_node].SetCurrentModel(best_model)
