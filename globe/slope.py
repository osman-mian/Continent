...
class Slope:
	...
	def FitSpline(self,source,target,M=1,temp=False):
		sse,coeff,hinge_count,interactions,arch_model,preds = rf.REarth(source,target,M)
		score = self.model_score(np.copy(coeff[0])) 
		return sse,score,coeff,hinge_count,interactions,arch_model,preds;
	...
	def model_score(self,coeff,debug=False):
